...
Use the Linear MCP. For Jira, run the same steps through the `use-tracker` skill's Jira adapter.

1. **Start with linked tickets.** If the seed commits or PRs reference ticket IDs (e.g., `ENG-1234`, `[BUG-567]`), fetch those first with `get_issue`. Read the full issue, and its comments with `list_comments`.
...
4. **Read project docs.** If the issue belongs to a project, use `get_project`, then `list_documents` and `get_document` for its docs. Project-level documents are where specs and rationale are most often captured.

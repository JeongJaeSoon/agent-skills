...
- A decision you wait on the human for (from your own analysis, a subagent, or a worker's mail) goes on the dashboard too: `orch decide add --title "<one line>" --body-file <file> --option "<label>::<what it means>"… [--recommend N] [--link URL]`, beside telling the human in your reply. It prints an id. The human answers in words or with the dashboard's buttons. A button records the answer in `decisions.json` and closes the decision itself, then types `decision <id>: <answer>` into your terminal once your turn is idle (a busy turn only delays it; `orch decide list` shows answers still waiting to be typed). Treat that line as the human's answer. An answer in words: `orch decide done <id> --answer "<answer>"`; on a decision the dashboard already closed, `done` only stops the pending line. When the question no longer matters, `orch decide drop <id>`. An open decision stays on the dashboard until it is closed.

...
같은 지시를 두 번 하게 되면 행동하기 전에 알맞은 목록에 한 줄을 먼저 추가한다. 워커 공통 목록만 모든 브리프의 STANDING에 그대로 붙는다. 워커 worktree 밖에 쓰게 하는 지시는 코디네이터 전용이다. 그런 원문은 코디네이터 전용에 그대로 두고, 워커 공통에는 "worker_done 본문에 적는다, 코디네이터가 옮긴다"로 바꾼 문장을 둔다.

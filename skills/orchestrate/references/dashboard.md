...
- **Item.** `#/fleet/item/<key>`: one Needs you item at full width with all of its text (line breaks kept) and its buttons. A click anywhere on a Needs you row outside its buttons, links and answer box opens it, for every type, including mail from a session Orca no longer lists. The chevron on the right of a row instead expands the same text in place, under the row, without leaving the list.

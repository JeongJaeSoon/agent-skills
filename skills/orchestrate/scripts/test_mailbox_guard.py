...
assert decide(f"bash -c 'ORCA_TERMINAL_HANDLE=term_other; {CHECK} --terminal $ORCA_TERMINAL_HANDLE --ack d1'") == "deny"
assert decide(f'{CHECK} --terminal "${{ORCA_TERMINAL_HANDLE:=term_other}}"') == "deny"
assert decide(f'[ "$ORCA_TERMINAL_HANDLE"=term_me ] && {CHECK} --terminal "$ORCA_TERMINAL_HANDLE"') == "pass"

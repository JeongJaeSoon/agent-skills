...
- `subagent_type`: `Explore` (the built-in read-only explorer)
...
Once all explorers have returned, spawn one `Agent` subagent to synthesize their findings into one explanation. Synthesize once; do not re-run it per explorer:

- `subagent_type`: `general-purpose`
- `model`: `opus`
- Tell it in the prompt that it only reads and writes the explanation; it must not edit files.

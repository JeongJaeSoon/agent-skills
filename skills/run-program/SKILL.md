...
5. **Drain.** Wait only with `prog.py wait <slug>` under `run_in_background` (it wakes on worker_done, escalation, or question, and acks heartbeat-only batches). Process every message in the batch, decide each settled worker's next owner (reuse, release), ack, then print `prog.py status` — its lines are how a drain ends.

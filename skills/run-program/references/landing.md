...
4. Record the verdict against the head that was reviewed: `prog.py verdict <slug> --pr N --sha <reviewed head> --source <who>`. It refuses when the PR's head has moved past that SHA. `<who>` names the non-author that produced it: `codex-review` (the worker's review loop), `verifier:codex` (a verifier unit), or `live:<verify-skill feature>`. A verdict from the author alone is not a verdict.
...
- `HOLD: behind base` → `gh pr update-branch N --repo R`, wait for CI, check again. If the patch-id changed (conflict resolution, a renumbered migration, even whitespace — the id is computed verbatim), the verdict is void: spawn a review or fix task.
...
Exit 0 → `record main_green --pr N --sha <merge commit>` (raises the cap once per commit). Non-zero → read the log; a real failure is `record main_red --pr N --sha <merge commit>` and one fix task, whose PR lands with `land-check --main-fix`; a flake gets `gh run rerun --failed` and a second watch. Then update the ticket (Linear Done with a completion comment, if the worker did not) and remove the landed worktree when its worker is released.
